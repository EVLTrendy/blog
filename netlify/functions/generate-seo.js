...
1.  **Keyword Collection**: Extract keywords from the content. Consider synonyms and related high-volume search terms in the niche (e.g., if content is about "JS", think "JavaScript", "Web Development", "Frontend").
2.  **Scoring & Filtering**: Prioritize keywords with high search intent and relevance. Remove generic filler words.
3.  **Clustering**: Group similar terms and pick the most potent one (e.g., choose "Next.js 14" over "Nextjs updates").
4.  **Generation**:
    *   **Title**: Create a compelling, click-worthy title (max 60 chars optimal). Include the primary keyword near the start.
    *   **Description**: Write a meta description (150-160 chars) that incites clicks (CTR) and includes secondary keywords.
    *   **Keywords**: A comma-separated list of the top 5-8 semantic keywords.
    *   **Image Alt**: A descriptive, keyword-rich alt text for a featured image representing this topic.
...
Generate the SEO metadata now.`;
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'llama3-70b-8192', // Using high-performance model for reasoning
        messages: [
